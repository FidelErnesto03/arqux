...
def test_project_bind_writes_session_to_brain(workspace_root: Path, governor_ctx) -> None:
    ...
    try:
        ...
        assert "# SESSIONS" in brain_text
        assert "agent=ex-1" in brain_text
        assert "role=executor" in brain_text
...
def test_project_unbind_marks_session_released_in_brain(workspace_root: Path, governor_ctx) -> None:
    ...
    try:
        ...
        assert "agent=ex-1" in brain_text
        assert "status=released" in brain_text
        # ex-2 should still be active.
        assert "agent=ex-2" in brain_text
        assert "status=active" in brain_text
    finally:
        os.chdir(cwd)
