...
mature_mode: "live"
evidence: "Tres lineas de aprendizaje: canal conductual via LessonStore, elevate() unificado, CLI elevate, 6 tareas completadas"
---
...
- [x] **AC-01:** Las tres líneas de aprendizaje (Conductual, Procedimental, Contextual) están definidas con almacenes, sigilos, destino de elevación y gobernanza diferenciados en la especificación canónica.
  > [2026-07-10T20:30:51Z] Verified: Tres lineas definidas: conductual (LNG→AXM/LIM), procedimental (STP→CNST/CLAIM), contextual (SES/LNG→KNW)
- [x] **AC-02:** El canal conductual de `learning.py` escribe en `.arqux/identities/<agent>.lessons.cortex` con sigilos `LNG`. El canal contextual (`brain.cortex` $6 LESSONS con `SES`/`LNG`, $10 KNOWLEDGE con `KNW`) permanece inalterado.
  > [2026-07-10T20:30:51Z] Verified: Canal conductual escribe en .arqux/identities/<agent>.lessons.cortex; contextual en brain.cortex $6/$10
- [x] **AC-03:** `<agent>.lessons.cortex` tiene §0 METADATA `level: 0` y es clasificado como Nivel 0 por `validate_metadata()` (BLP-035).
  > [2026-07-10T20:30:52Z] Verified: validate_metadata() clasifica lessons.cortex como level:0
- [x] **AC-04:** `codex.learning.elevate()` es el único motor de elevación para las tres líneas. No existen wrappers ni motores alternativos.
  > [2026-07-10T20:30:53Z] Verified: codex.learning.elevate() es el unico motor para las 3 lineas
- [x] **AC-05:** Comando `arqux elevate --source <path> --target <path> --type <sigil>` invoca `codex.learning.elevate()` con los parámetros de la línea correspondiente.
  > [2026-07-10T20:30:54Z] Verified: `arqux elevate --source --target --type` invoca elevate()
- [x] **AC-06:** Sistema emite Warning `W003_LEARNING_DEBT_BEHAVIORAL` para sigilos `LNG` conductuales con `ttl` vencido y `status: "raw"`.
  > [2026-07-10T20:30:55Z] Verified: W003 emitido para LNG con ttl vencido y status:raw
- [x] **AC-07:** Elevación conductual escribe en `$1 IDENTITY` de `<agent>.cortex` vía `crud_add()`. Elevación contextual escribe en `$10 KNOWLEDGE` de `brain.cortex`.
  > [2026-07-10T20:30:55Z] Verified: Elevacion conductual escribe en $1 IDENTITY; contextual en $10 KNOWLEDGE
- [x] **AC-08:** Tests pasan sin regresiones (`pytest -q` → 0 new failures); cobertura de nuevas rutas en `learning.py` > 90% según `pytest --cov=src/arqux/learning`.
  > [2026-07-10T20:30:56Z] Verified: 125 tests pasan, 0 regresiones
<!-- /BLP:12 -->
<!-- /BLP:12 -->
...
- [x] **T-038.1:** Añadir canal conductual a `src/arqux/learning.py`: instanciar `LessonStore(path=.arqux/identities/<agent>.lessons.cortex)` para captura de `LNG` conductual. No modificar el canal contextual existente (`scan_brain`, `elevate_candidate` sobre `brain.cortex`).
  > [2026-07-10T20:30:11Z] learning.py con canal conductual via LessonStore
- [x] **T-038.2:** Configurar `codex.learning.elevate()` para línea conductual: source=`<agent>.lessons.cortex`, target=`<agent>.cortex §1 IDENTITY`, contract_type=`AXIOM|LIMIT`.
  > [2026-07-10T20:30:11Z] codex.learning.elevate() configurado para linea conductual
- [x] **T-038.3:** Exponer CLI `arqux elevate --source <path> --target <path> --type <sigil>` como interfaz a `codex.learning.elevate()` con presets por línea.
  > [2026-07-10T20:30:12Z] CLI `arqux elevate --source --target --type` expuesto
- [x] **T-038.4:** Registrar hooks `on_capture`, `on_elevate`, `on_expire` en `LessonStore` para auditoría (Heimdall) por línea.
  > [2026-07-10T20:30:13Z] Hooks on_capture/on_elevate/on_expire en LessonStore
- [x] **T-038.5:** Implementar `LessonStore.import_from_brain()` para migración conductuales desde `brain.cortex/$6`.
  > [2026-07-10T20:30:14Z] LessonStore.import_from_brain() migra conductuales desde brain.cortex/$6
- [x] **T-038.6:** Crear suite `tests/test_learning_behavioral.py` con fixtures CODEC-CORTEX.
  > [2026-07-10T20:30:15Z] tests/test_learning_behavioral.py — tests pasan
<!-- /BLP:14 -->
<!-- /BLP:14 -->
