...
- [ ] **AC-01:** `IdentityManager` implementa `IIdentityResolver` con `resolve()`, `bind_to_session()`, `elevate_to_identity()`
- [ ] **AC-02:** `bind_to_session("jarvis")` carga `jarvis.cortex`, establece `session.context.agent = "jarvis"` y expone sus contratos `AXM`/`LIM`
- [ ] **AC-03:** `elevate_to_identity("jarvis", "lsn-042", "AXIOM")` inyecta `AXM:lsn-042{...}` en `$1 IDENTITY` de `jarvis.cortex`
- [ ] **AC-04:** Resolver identidad inexistente lanza `IdentityNotFoundError` y usa `alfred` por defecto
- [ ] **AC-05:** `IdentityManager` resuelve identidades de agente. La interfaz `IIdentityResolver` permite futura implementación `UserIdentityResolver` sin modificar la clase base
- [ ] **AC-06:** Tests pasan sin regresiones; cobertura > 85%
<!-- /BLP:12 -->
...
- [ ] **T-039.1:** Implementar `IIdentityResolver` interface + `IdentityManager` class en `src/arqux/identity.py`
- [ ] **T-039.2:** Implementar `resolve(name)` — carga archivo `.cortex`, valida metadata, retorna `CortexArtifact`
- [ ] **T-039.3:** Implementar `bind_to_session(name)` — resuelve + vincula a `session.context`
- [ ] **T-039.4:** Implementar `elevate_to_identity(agent, lesson_id, contract_type)` — inyección de AXM/LIM en `$1 IDENTITY`
- [ ] **T-039.5:** Exponer CLI `arqux identity resolve <name>` y `arqux identity elevate --agent <name> <lesson_id> --type AXIOM|LIMIT`
- [ ] **T-039.6:** Crear suite `tests/test_identity.py` con fixtures de identidades
<!-- /BLP:14 -->
