...
updated_at: "2026-07-07T15:08:39Z"
closed_at: ""
priority: "high"
complexity: "standard"
quality_gates: ""
...
- [ ] **AC-01:** `session.close()` genera SES en brain PULSE con BLPs activos y status
- [ ] **AC-02:** `session.resume()` retorna el último SES con todas las claves pobladas
- [ ] **AC-03:** `session.status()` retorna metadata sin restaurar contexto completo
- [ ] **AC-04:** SES < 2KB incluso con 10+ BLPs activos
- [ ] **AC-05:** `adoption.skill.md §6` incluye paso de session.resume()
- [ ] **AC-06:** Tests existentes pasan
- [ ] **AC-07:** Prueba real: cerrar esta sesión, abrir nueva, verificar contexto
...
- [ ] **T-1.1:** Implementar `session.close` en handlers/session.py
- [ ] **T-1.2:** Implementar `session.resume` en handlers/session.py
- [ ] **T-1.3:** Implementar `session.status` en handlers/session.py
- [ ] **T-2.1:** Definir estructura SES en CORTEX (< 2KB)
- [ ] **T-3.1:** Actualizar `adoption.skill.md §6` con session.resume
- [ ] **T-4.1:** Tests para close, resume, status
- [ ] **T-4.2:** Prueba real de handoff entre sesiones
