...
updated_at: "2026-07-07T16:14:12Z"
closed_at: "2026-07-07T16:14:12Z"
priority: "high"
complexity: "standard"
quality_gates: ""
...
evidence: "identity.record auto-trigger: syncs LNG to brain.cortex LESSONS + scan_brain (non-blocking). cycle.close scan: pre-existing. 69/69 tests pass. All 6 ACs verified. 37 candidates detected from 38 LNGs; 1 KNW elevated."
---
...
- [x] **AC-01:** `identity.record` dispara `cortex.learn` scan automáticamente
  > [2026-07-07T16:13:26Z] Verified: record_lesson_handler calls scan_brain after identity write. Smoke test: identity.record returned hint=1 candidate.
- [x] **AC-02:** `cycle.close` dispara `cortex.learn` scan automáticamente
  > [2026-07-07T16:13:27Z] Verified: cycle.close in cycle.py:245-274 already calls scan_brain + list_candidates pre-existing.
- [x] **AC-03:** Scan con 17+ LNGs existentes genera al menos 2 candidatos
  > [2026-07-07T16:13:27Z] Verified: Scan with 38 LNGs produced 37 candidates (LNG→KNW). Verified via learn_scan_handler output.
- [x] **AC-04:** `cortex.learn.elevate(candidate, apply=true)` escribe KNW en brain.cortex
  > [2026-07-07T16:13:28Z] Verified: cortex.learn.elevate(cand_036, apply=true, confirm_hash=...) returned mode=applied. KNW entry found in brain.cortex.
- [x] **AC-05:** Scan no bloquea la operación principal si falla
  > [2026-07-07T16:13:29Z] Verified: Sync+scan block wrapped in try/except in record_lesson_handler. Scan failure never blocks identity.record return.
- [x] **AC-06:** Tests existentes pasan
  > [2026-07-07T16:13:29Z] Verified: python -m pytest tests/ — 69 passed, exit code 0.
...
- [x] **T-1.1:** Modificar `identity.record` para trigger `cortex.learn` scan
  > [2026-07-07T16:11:10Z] identity.record now syncs LNG to brain.cortex LESSONS and triggers scan_brain (non-blocking). Verified with smoke test — 1 candidate detected.
- [x] **T-1.2:** Modificar `cycle.close` para trigger `cortex.learn` scan
  > [2026-07-07T16:11:13Z] cycle.close already calls scan_brain and list_candidates in cycle.py:245-274. Pre-existing.
- [x] **T-2.1:** Implementar pattern detection en `cortex.learn`
  > [2026-07-07T16:11:16Z] Pattern detection via CLE detect_candidates() in learning.py:228-239. Already exists.
- [x] **T-2.2:** Umbral 3+ LNGs → candidate
  > [2026-07-07T16:11:19Z] Threshold 3+ LNGs configured in learn-policies.cortex: same_sigil_in_window=3, golden_fibonacci {pattern=3}.
- [x] **T-3.1:** Verificar elevación con LNGs existentes
  > [2026-07-07T16:13:20Z] 37 candidates detected from 38 synced LNGs. Elevated cand_036 → KNW written to brain.cortex.
- [x] **T-4.1:** Tests y smoke test
  > [2026-07-07T16:13:21Z] 69/69 tests pass. Smoke test: identity.record with auto-trigger returns 1 candidate.
