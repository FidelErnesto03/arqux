...
| `cortex.gc` | Garbage-collect duplicate entries in a .cortex file. Duplicates share the same (section, sigil, name). dry_run=True (default) previews without mutating; force=True removes duplicates conserving the first first_kept occurrences (default 1). Output metrics: bytes_written/file_bytes = whole file size after the rewrite (unlike entry.add, where bytes_written is the serialized entry size). |
